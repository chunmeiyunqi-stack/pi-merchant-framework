...
const Packer = (Docx as any).Packer;
const Paragraph = (Docx as any).Paragraph;
const TextRun = (Docx as any).TextRun;
const AlignmentType = (Docx as any).AlignmentType;
const Header = (Docx as any).Header;
const Footer = (Docx as any).Footer;
const PageNumber = (Docx as any).PageNumber;
const TextWrappingType = (Docx as any).TextWrappingType;
const ImageRun = (Docx as any).ImageRun;
const HeadingLevel = (Docx as any).HeadingLevel;
const BorderStyle = (Docx as any).BorderStyle;
...
const SOURCE_CONFIG = {
  scanDirs: ['packages/pi-sdk/src', 'apps/web/src/app', 'apps/admin/src/app'],
...
async function generateUserManualDoc() {
    ...
    lines.slice(1).forEach((line) => {
      if (line.startsWith('## ')) {
        docChildren.push(
          new Paragraph({
            children: [
              new TextRun({ text: line.replace('## ', ''), bold: true, size: 32, font: '黑体' }),
            ],
        ...
        docChildren.push(
          new Paragraph({
            children: [
              new TextRun({ text: line.replace('### ', ''), bold: true, size: 28, font: '黑体' }),
            ],
  ...
  if (fs.existsSync(screenshotDir)) {
    ...
    docChildren.push(
      new Paragraph({
        children: [
          new TextRun({ text: '附录：软件运行效果图', bold: true, size: 32, font: '黑体' }),
        ],
        spacing: { before: 400 },
      })
    );
    ...
    images.forEach((img, index) => {
      ...
      docChildren.push(
        new Paragraph({
          children: [
            new TextRun({
              text: `图 ${index + 1}：${img.replace('.png', '')}`,
              size: 18,
              font: '宋体',
            }),
          ],
