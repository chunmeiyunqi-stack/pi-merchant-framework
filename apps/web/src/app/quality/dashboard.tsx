...
export function CodeQualityDashboard() {
  ...
  if (loading) return <div className="p-8 text-center text-gray-500">Loading quality statistics...</div>;
...
function PythonServiceStatus() {
  ...
  useEffect(() => {
    pythonClient.healthCheck()
      .then((r) => setStatus(r.longcat_connected ? 'Connected (LongCat ready)' : 'Connected (no API key)'))
      .catch(() => setStatus('Unreachable'));
  }, []);

  return (
    <div className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 rounded-lg px-4 py-2">
      <span className={`inline-block w-2 h-2 rounded-full ${status.startsWith('Connected') ? 'bg-green-500' : 'bg-red-500'}`} />
...
function ScoreDistribution({ distribution, total }: { distribution: StatisticsData['score_distribution']; total: number }) {
  return (
    <section>
              ...
              <tr key={s.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                ...
                <td className="p-3">
                  <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${
                    parseFloat(s.overall_score) >= 7 ? 'bg-green-100 text-green-800'
                    : parseFloat(s.overall_score) >= 5 ? 'bg-yellow-100 text-yellow-800'
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {data.map((d) => (
          <div key={d.language} className="bg-white rounded-xl border border-gray-200 p-4 flex justify-between items-center">
            <div>
