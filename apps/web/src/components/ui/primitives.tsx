...
export function StatCard({
  ...
  return (
    <Card hover className="relative overflow-hidden group">
      <div
        className={cn(
          'absolute right-0 top-0 w-16 h-16 rounded-bl-full flex items-start justify-end p-3',
          c.bg
        )}
      >
      ...
      {action && (
        <a
          href={action.href}
          className="text-sm font-semibold text-brand-purple hover:text-brand-purple-hover transition-colors"
        >
          {action.label} �?{' '}
        </a>
      )}
