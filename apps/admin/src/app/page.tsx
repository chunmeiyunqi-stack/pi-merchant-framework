      ...
      >
        <div
          style={{
            ...
            border: '1px solid rgba(243,193,54,0.2)',
            borderRadius: 20,
            padding: '40px 48px',
            width: 360,
            textAlign: 'center',
        ...
        >
          ...
          <h2 style={{ color: '#f0e6ff', fontSize: 20, fontWeight: 700, marginBottom: 6 }}>
            商户管理后台
          </h2>
          <p style={{ color: 'rgba(240,230,255,0.45)', fontSize: 13, marginBottom: 28 }}>
            美丽时光工作室 · Admin
          </p>
          <input
            type="password"
            ...
            onKeyDown={(e) =>
              e.key === 'Enter' && (pwd === 'admin888' ? setAuthed(true) : setErr(true))
            }
            style={{
              width: '100%',
              padding: '11px 16px',
              ...
              border: err ? '1px solid rgba(192,57,43,0.6)' : '1px solid rgba(243,193,54,0.2)',
              color: '#f0e6ff',
              fontSize: 14,
              outline: 'none',
          ...
          />
          {err && (
            <p style={{ color: '#e74c3c', fontSize: 12, marginBottom: 10 }}>密码错误，请重试</p>
          )}
          <button
            onClick={() => (pwd === 'admin888' ? setAuthed(true) : setErr(true))}
            style={{
              width: '100%',
              padding: '12px',
              borderRadius: 12,
          ...
          <p style={{ color: 'rgba(240,230,255,0.25)', fontSize: 11, marginTop: 16 }}>
            提示：密码 admin888
          </p>
        </div>
      </div>
  ...
  return (
    ...
    >
        ...
        <div style={{ display: 'flex', gap: 24 }}>
          {tabs.map((t) => (
            <button
              ...
              style={{
                ...s,
                color: activeTab === t ? '#F3C136' : 'rgba(240,230,255,0.5)',
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
        ...
        {activeTab === '概览' && (
          <div>
            <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 24, color: '#f0e6ff' }}>
              📊 今日概览
            </h2>
            <div
              style={{
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: 'rgba(240,230,255,0.4)' }}>
                      Pi Network 用户
                    </div>
                  </div>
                </div>
                <span style={{ color: 'rgba(240,230,255,0.45)', fontSize: 13 }}>
                  {i + 1} 笔订单
                </span>
              </div>
            ))}
...
function OrderTable({ orders }: { orders: typeof mockOrders }) {
  ...
  const label = (s: string) =>
    s === 'completed' ? '已完成' : s === 'pending' ? '待处理' : '已取消';
  return (
    <div
      style={{ borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.07)' }}
    >
      <div
        style={{
          display: 'grid',
          gridTemplateColumns: '1fr 1.2fr 1.5fr 0.8fr 0.9fr 0.7fr',
          padding: '12px 20px',
          background: 'rgba(255,255,255,0.04)',
          fontSize: 12,
