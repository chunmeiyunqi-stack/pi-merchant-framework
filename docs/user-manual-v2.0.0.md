...
4. [核心功能](#4-核心功能)
   ...
   - 4.3 [智能 AI 路由系统 (V1.1.0)](#43-智能-ai-路由系统-v110-新增)
   - 4.4 [Pi Network 支付集成](#44-pi-network-支付集成)
   - 4.5 [商户配置化引擎](#45-商户配置化引擎)
5. [商业化核心能力 (V2.0.0 新增)](#5-商业化核心能力-v200-新增)
   - 5.1 [License 授权验证系统](#51-license-授权验证系统)
   - 5.2 [多租户架构](#52-多租户架构)
   - 5.3 [用量统计与配额管理](#53-用量统计与配额管理)
6. [运维管理](#6-运维管理)
   - 6.1 [日志与监控](#61-日志与监控)
   - 6.2 [数据库维护](#62-数据库维护)
   - 6.3 [常见运维操作](#63-常见运维操作)
7. [附录](#7-附录)
   - 7.1 [术语表](#71-术语表)
   - 7.2 [文档修订记录](#72-文档修订记录)
...
## 4.8 合规与法律声明 (V2.0.0 新增)

本节概述 V2.0.0 版本中新加入的合规与法律声明模块，包含数据隐私保护架构、AI 内容免责声明、及链上交易责任说明。

### 4.8.1 隐私保护架构

- **最小化数据收集**：平台仅收集商户必要的标识（merchantId）与用于会话验证的 HMAC 签名会话令牌（Cookie 名称：`pi_auth_token`）。平台不会存储用户私钥或支付密码，也不以明文形式在第三方云端存储会话凭证。
...
- **AI 生成内容免责声明**：平台提供的 AI 建议与输出仅作参考，用户在依据 AI 给出的建议进行决策前应自行验证其适用性与准确性。平台在法律允许范围内对因依赖 AI 内容导致的任何损失不承担额外责任。
...
本节内容与代码实现对应：`packages/pi-sdk/src/tenant/context.ts`（AsyncLocalStorage 租户上下文），`packages/pi-sdk/src/tenant/prisma-middleware.ts`（Prisma 中间件注入 `merchantId`），以及中间件与 auth 路由中对 Cookie 名称 `pi_auth_token` 的统一使用。

# 5 商业化核心能力 (V2.0.0 新增)

本系统在 V2.0.0 版本中引入了企业级商业化核心能力，为规模化 SaaS 运营提供底层支撑，包含 License 授权验证、多租户隔离架构以及用量统计管控。

## 5.1 License 授权验证系统

采用去中心化的加密 License 验证机制，支持多层级商业授权控制。

- **数字签名验证**：使用 HMAC-SHA256 验证 License 签名，防止篡改与伪造。
- **Feature Gate（功能开关）**：根据套餐等级（Starter / Professional / Enterprise）动态控制 `ai_routing`, `multi_tenant` 等核心特性的访问权限。
- **生命周期管理**：强制过期检查，临期警告日志记录。
- **环境自适应**：在 `NODE_ENV=development` 模式下自动颁发企业级开发许可，保障本地研发不被阻断。

## 5.2 多租户架构

实现商户级数据与配置硬隔离，支持单一框架实例服务海量商户（Tenant）。

- **生命周期状态机**：支持租户的 Active（激活）、Suspended（挂起）与 Cancelled（注销）状态管理。
- **严格容量控制**：基于 License 限定的 `maxTenants` 强制控制最大实例数。
- **独立 AI 配置空间**：允许每个租户独立覆盖系统默认的 `primaryProvider`、容错队列及 Token 限制。

## 5.3 用量统计与配额管理

提供微秒级的 API 调用与 Token 消耗追踪，支撑月度订阅制计费模型。
...
| V2.0.0 | 2026-05-15 | 新增商业化核心能力：License 授权验证系统（HMAC-SHA256 + Feature Gate）、多租户架构（CRUD + 配额控制 + 状态隔离）、用量统计与 Webhook Flush；代码质量加固（TypeScript 严格模式，190/190 测试全通过）。补充数据隐私保护模块与商业免责声明，符合 GDPR 类合规标准。 | 技术团队 |
